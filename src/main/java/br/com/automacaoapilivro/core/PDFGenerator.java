package br.com.automacaoapilivro.core;
...
public class PDFGenerator {
	...
	public void iniciaPDF(Scenario cenario) throws Exception {
		...
		PdfWriter.getInstance(document,new FileOutputStream((pathtoFolder + "/" + "  AMBIENTE[UAT]  " + cenario.getName()+ " " +converterDataParaString.replace("/","-").replace(":", "h")  +" " + ".pdf")));

		document.open();
	

		String[] headers = new String[] { "ENV", "Feature", "Cenario", "Data" };
		String[][] rows = new String[][] {{ "uat"+ " - OS: " + getOperatingSystem(), featureName,
		...
		try {
			...
			Image logo = Image.getInstance("./src/main/report/report.png");
